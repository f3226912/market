...
public class DateUtil  extends DateUtils {
	 ...
	 * @return
	 */
    public static int getDaysByDate(String dateStr, String fmtStr){ 
    	Date date = formateDate(dateStr, fmtStr);
    ...
    public static int getDaysByDate(int year, int month){
    	...
    	Date date = formateDate(dateStr, DateUtil.sdfDateOnly);
        Calendar now = Calendar.getInstance();
        now.setTime(date);
        int day=now.getActualMaximum(Calendar.DATE);
		return day;
     ...
     * @return
     */
    public static int getDayOfDate(String dateStr, String fmtStr){
    	Date date = formateDate(dateStr, fmtStr);
    	Calendar now = Calendar.getInstance();
    	now.setTime(date);
     ...
     * @return
     */
    public static int getMonthOfDate(String dateStr, String fmtStr){
    	Date date = formateDate(dateStr, fmtStr);
    	Calendar now = Calendar.getInstance();
    	now.setTime(date);
     ...
     * @return
     */
    public static int getYearOfDate(String dateStr, String fmtStr){
    	Date date = formateDate(dateStr, fmtStr);
    	Calendar now = Calendar.getInstance();
    	now.setTime(date);
    ...
    public static String getDateAfter(Date date,int day){  
     ...
     return toString(now.getTime(),DateUtil.DATE_FORMAT_DATETIME);
    }  
    /**
     * 根据当前日期获取前N个月的日期
     * @return
    ...
    public static String getDateMonth(int month){
     ...
     return toString(cal.getTime(),DateUtil.DATE_FORMAT_DATETIME);
    }
    ...
    /** 获取当前日期的最后时间, 精确到毫秒
     * @param inputDate
     * @param inputDateType 1-dateOnly, 2-dateTime
     * @return
     */
    public static Date getEndOfDay(String inputDate, int inputDateType) {
    	Date res = null ;
    	if (StringUtils.isEmpty(inputDate)){
    		...
    		try {
    			if (1 == inputDateType){
    				res = getEndOfDay(sdfDateOnly.parse(inputDate));
    			}else if (2 == inputDateType){
    				res = getEndOfDay(sdfDateTime.parse(inputDate));
    			}
     ...
     * @return
     */
    public static Date getStartOfDay(String inputDate, int inputDateType) {
    	Date res = null ;
    	if (StringUtils.isEmpty(inputDate)){
    		res = null ;
    	}else{
    		try {
    			if (1 == inputDateType){
    				res = getStartOfDay(sdfDateOnly.parse(inputDate));
    			}else if (2 == inputDateType){
    				res = getStartOfDay(sdfDateTime.parse(inputDate));
    			}
			} catch (ParseException e) {
				res = null ;
			}
	...
	public static void main(String[] args) {
		System.out.println(formateDate("2016-3-31"));
		System.out.println(getLastDayOfMonth(2016, 3));
	}
}
