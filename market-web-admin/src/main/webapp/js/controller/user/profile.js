		...
		$("#form-edit").validate({
			...
			submitHandler: function (form) {
				$(form).ajaxSubmit({
					type: "POST",
					dataType:"json",
					url:CONTEXT+"sysPlatUser/profile",
		...
		$("#form-pwd").validate({
			...
			submitHandler: function (form) {
				$(form).ajaxSubmit({
					type: "POST",
					dataType:"json",
					url:CONTEXT+"sysPlatUser/setting",
					success: function (data) {
						if(data.success){
...
//			var addData = $('#form-edit').serialize();
//			$.post(CONTEXT+"sysPlatUser/profile",addData,function(data){
//				if(data.success){
//					$.eAlert("我的帐号","修改账号信息成功");
//				} else {
...
//			$.post(CONTEXT+"sysPlatUser/setting",addData,function(data){
//				if(data.success){
//					$.eAlert("修改密码","修改密码成功,请重新登录");
////					alert("修改密码成功,请重新登录");
//					window.location="logout"
