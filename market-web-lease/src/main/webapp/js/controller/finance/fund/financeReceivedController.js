function _call(templateUrl,param){
	...
	function(){
		...
		function loadData(page, isInit){
			$.getJSON(CONTEXT+"financeHas/queryFeeReceived",page,function(data){
				if(data.success){
					if(isInit){
		...
		//绑定导出事件
		$("#financeHasExport").click(function(){
			if(!Route.market){
			...
			disableExport = true;
			$.ajax({
				url: CONTEXT+'financeHas/checkExportParams',
				data : condition,
				type:'post',
				success : function(data){
					...
					if (data && data.success){
						//启动下载
						$.download(CONTEXT+'financeHas/exportData',paramList,'post' );
					}else{
