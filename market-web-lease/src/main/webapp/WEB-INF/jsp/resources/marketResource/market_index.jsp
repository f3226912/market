...
<div class="system-modal market-style">
	<section class="header">
		...
		<div class="right-icon"> 
		    ...
		    <gd:btn btncode="BtnMarketResourceCarAdd">
			 <a href="javascript:(function(){if(!Route.market){$.eAlert('提示','当前公司下未关联市场，暂不能执行添加操作！');return;}Route.params={removeMarketType:'carResource',currentype:'marketResource'};location.href='index#viewMarketResource';})();" ><i class="fa fa-plus-square"></i>新增商铺</a>
			</gd:btn>
			<gd:btn btncode="BtnMarketResourceCarRelease">
			 <a href="javascript:void(0)" id="release"><i class="fa fa-external-link"></i>放租</a>
			</gd:btn>
			<gd:btn btncode="BtnMarketResourceCarBack">
			<a href="javascript:void(0)" id="receive"><i class="fa fa-retweet"></i>回收</a>
			</gd:btn>
			<gd:btn btncode="BtnMarketResourceCarDelete">
	...
	<div class="form-page">	
		<div class="adv-table editable-table resoure-table">
			<table class="table  table-hover t-custom" id="tb">
				<thead>
					<tr class="ml-ct">
						<th width="6%"><input type="checkbox" id="checkAll" class="checkbox-range" value=""><span class="checkbox-span"></span></th>
						<th width="6%">序号</th>
						<th width="12%">资源编码</th>
						<th width="12%">租赁资源名称</th>
						<th width="12%">所属楼栋</th>
						<th width="12%">所属区域</th>
						<th width="10%">建筑面积</th>
						<th width="10%">套内面积</th>
									...
									<div class="form-page">
										<form id="form-setResource" method="post" novalidate
											...
											<div class="form-group col-lg-5">
												...
												<div class="col-md-6 text-left">
													<input type="text" name="resourceTypeName" readonly class="form-control" value="">
													<input name="resourceTypeId" type="hidden" value="1">
												</div>
											</div>
											<div class="form-group col-lg-5">
												<label class="control-label col-md-6"> <i
...
<script id="marketResourceScript" type="text/x-jquery-tmpl">
	{{each(i, marketResource) marketResources}}
		<tr class="ml-ct">
			<td class="text-center"><input id="checkbox" type="checkbox" class="checkbox-range" value="${marketResource.id}"/><span class="checkbox-span"></span></td>
			<td>${i+1}</td>
			<td title="${marketResource.code}">${marketResource.code}</td>
			<td title="${marketResource.name}"><a href="javascript:(function(){Route.params={removeMarketType:'carResource',currentype:'marketResource',id:${marketResource.id}};location.href='index#viewMarketResource';})();" style="color: #664de8;">${marketResource.name}</a></td>
			<td title="${marketResource.buildingName}">${marketResource.buildingName}</td>
			<td>${marketResource.areaName}</td>
			<td>${marketResource.builtArea}</td>
			<td>${marketResource.innerArea}</td>
