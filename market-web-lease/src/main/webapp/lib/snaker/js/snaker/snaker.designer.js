(function(snaker){
	...
	designer.config={
		...
		path:{
			...
			textPos:{x:0,y:-10},
			props:{
				name:{name:"name",label:"标识",value:"",editor:function(){return new designer.editors.inputEditor()}},
    ...
    designer.util = {
        ...
        nextId : (function() {
        	//=====更换新的UUID计算方式 ，保证全局唯一by wjguo 2016.11.08==
        	var curTime = new Date().getTime();
            return function() {
            	var uuid = 'xxxxx'.replace(/[xy]/g, function(c) {
        		  var r = (curTime + Math.random()*16)%16 | 0;
...
/*                var returnValue = window.showModalDialog(designer.config.ctxPath + '/config/form?lookup=1',window,'dialogWidth:1000px;dialogHeight:600px');
        ...
        this.toBeforeXml=function(){
            ...
            for(var k in _o.props){
            	//=== 必须要有显示名称。 by wjguo 2016.10.25===
            	if(k=="displayName"&&_o.props[k].value==""){
            		alert(_o.type+" 显示名称 不能为空");
            		return ""
            	}
            	if(k=="name"&&_o.props[k].value==""){
            		alert(_o.type+" 标识 不能为空");
            		return ""
            	}
            	if(k=="assignee"&&_o.props[k].value==""){
            		alert(_o.type+" 必须指定审批者岗位");
            		return ""
            	}
	...
	designer.init=function(c, o){
		...
		if(designer.config.editable){
			...
			//给顶级的父window绑定获取的流程定义
			window.top.getSnakerProcessDefine = function(){
				var i="<process ";
				for(var c in designer.config.props.props){
					/* ===流程的名称不在设计图中进行操作。by wjguo 2016.10.25==
				...
				for(var node in _states){
					if(_states[node]){
						//=== 如果数据构造出问题，则跳出当前方法。  by wjguo 2016.10.25===
						var beforeXml = _states[node].toBeforeXml();
						if (!beforeXml) {
							return false;
						}
						i+= beforeXml;
						...
						i+="\n";
						//=== 如果数据构造出问题，则跳出当前方法。  by wjguo 2016.10.25===
						var afterXml = _states[node].toAfterXml();
						if (!afterXml) {
							return false;
						}
						i+= afterXml;
				...
				if (i.indexOf("<task") == -1) {
					alert("流程步骤没有定义任何任务节点");
					return false;
				}
				if (i.indexOf("<transition") == -1) {
					alert("流程步骤节点没有任何的关联线路");
					return false;
				}
				
				return i;
			}
			//===不通过保存按钮来执行。by wjguo 2016.10.25==
